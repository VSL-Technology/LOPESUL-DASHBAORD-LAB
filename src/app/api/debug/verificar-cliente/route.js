...
export async function GET(req) {
  try {
    if (!checkInternalAuth(req)) {
      logger.warn({}, '[debug/verificar-cliente] acesso negado (internal token)');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const validation = validateInternalToken(req);
    if (!validation.ok) {
      logger.warn({ reason: validation.reason }, '[debug/verificar-cliente] acesso negado');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    ...
    logger.error({ error: error?.message || error }, '[debug/verificar-cliente] erro');
    return NextResponse.json(
      {
        ok: false,
        error: error.message,
