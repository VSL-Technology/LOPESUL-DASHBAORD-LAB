...
export async function processAudit(audit) {
  try {
    ...
    if (mikrotikId) {
      ...
      // Filter by mikrotikId in payload client-side
      const matched = rows.filter((r) => r.payload?.mikrotikId === mikrotikId);
      if (matched.length >= 3) {
    ...
    // Rule B: distributed fails across multiple mikrotiks: >=10 FAIL in 10 minutes across >=3 mikrotikIds
    {
      ...
      const rows = await prisma.auditEntry.findMany({
        where: { action: 'MIKROTIK_RELEASE_FAIL', AND: [{ createdAt: { gte: since } }] },
        orderBy: { createdAt: 'desc' },
        take: 200,
      });
      if (rows.length >= 10) {
        const grouping = {};
        for (const r of rows) {
          const mid = r.payload?.mikrotikId || 'unknown';
    ...
    if (orderCode) {
      ...
      const seq = rows
        .filter((r) => r.payload?.orderCode === orderCode)
        .map((r) => ({ id: r.id, event: r.action, ts: r.createdAt }));
      if (seq.length >= 2) {
