...
2. Cada Mikrotik deve estar acessível via SSH (usuário/senha definidos em `MIKROTIK_USER` / `MIKROTIK_PASS` no `.env`)
...
```bash
ssh "${MIKROTIK_USER}"@10.200.200.2  # Ajuste o IP (credenciais no .env)
```
