...
for IP in "${!MIKROTIKS[@]}"; do
  ...
  echo "$REDIRECT_HTML" | sshpass -p "$MIKROTIK_PASS" ssh -o StrictHostKeyChecking=no "$MIKROTIK_USER"@$IP \
  ...
  sshpass -p "$MIKROTIK_PASS" ssh -o StrictHostKeyChecking=no "$MIKROTIK_USER"@$IP \
