...
MIKROTIK="${MIKROTIK_HOST:-${MIKROTIK_IP:-10.200.200.7}}"
...
RELAY_URL="${RELAY_URL:?Defina RELAY_URL no ambiente/.env}"
...
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER_VAR"@$MIKROTIK << EOF 2>/dev/null
...
if [ $? -ne 0 ]; then
  ...
  curl -X POST "$RELAY_URL/relay/exec" \
    -H "Content-Type: application/json" \
    -d "{
      \"host\": \"$MIKROTIK\",
      \"user\": \"$MIKROTIK_USER_VAR\",
      \"pass\": \"$MIKROTIK_PASS_VAR\",
...
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER_VAR"@$MIKROTIK \
