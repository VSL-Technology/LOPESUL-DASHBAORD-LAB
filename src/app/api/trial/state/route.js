...
import { garantirEstadoMikrotik } from '@/lib/mikrotikAccessControl';
...
export async function POST(req) {
  try {
    ...
    logger.info({ mac, ip }, '[trial/state] consultando');
    ...
    logger.error({ error: error?.message || error }, '[trial/state] erro');
    return NextResponse.json({ success: false, error: 'Erro interno ao consultar trial' }, { status: 500 });
  }
}
