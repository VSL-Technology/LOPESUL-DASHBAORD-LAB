...
RELAY_RESPONSE=$(curl -s -X POST "$RELAY_URL/relay/exec" \
...
BINDING_RESPONSE=$(curl -s -X POST "$RELAY_URL/relay/exec" \
  -H "Content-Type: application/json" \
  -d "{
    \"host\": \"$MIKROTIK\",
    \"user\": \"$MIKROTIK_USER_VAR\",
...
ACTIVE_RESPONSE=$(curl -s -X POST "$RELAY_URL/relay/exec" \
  -H "Content-Type: application/json" \
  -d "{
    \"host\": \"$MIKROTIK\",
    \"user\": \"$MIKROTIK_USER_VAR\",
