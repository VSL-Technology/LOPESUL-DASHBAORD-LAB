    ...
    (async () => {
      try {
        const [rDash, rPays, rSess, rTrial] = await Promise.allSettled([
          ...
          fetchJSON(`/api/sessoes?trial=true&limit=10`, { timeoutMs: SESS_TIMEOUT, signal: ac.signal }),
        ]);
        ...
        if (rTrial.status === "fulfilled" && rTrial.value?.ok) {
          const lista = Array.isArray(rTrial.value.data) ? rTrial.value.data : [];
          setTrialSessions(lista);
          setCache("dash:trial:v1", lista);
        }
      } catch (e) {
        console.error(e);
      ...
      <div className="p-6 md:p-8 bg-[#F0F6FA] dark:bg-[#1a2233] min-h-screen transition-colors">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {[
            ...
            { title: "Trials Ativos", value: kpis.trials },
          ].map(({ title, value }) => (
            <div
              key={title}
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-[#232e47] p-4 rounded-xl shadow transition-colors">
              <h3 className="font-semibold mb-2 text-gray-800 dark:text-white">Trials em andamento</h3>
              {loading ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">Carregando…</p>
              ) : trialSessions.length ? (
                <ul className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                  {trialSessions.map((sessao) => (
                    <li key={sessao.id} className="border-b border-gray-200 dark:border-gray-700 pb-2">
                      <div className="font-semibold">{sessao.nome || sessao.ipCliente || sessao.macCliente || 'Cliente'}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        Expira em {sessao.expiraEm ? fmtData(sessao.expiraEm) : '—'}
                      </div>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum cliente em trial.</p>
              )}
            </div>
