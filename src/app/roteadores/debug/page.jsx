  ...
  return (
    <div className="p-6 space-y-4 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex gap-2 items-center">
        ...
        {loading && <span className="text-sm text-gray-600 dark:text-slate-300">Carregando roteadores...</span>}
      </div>

      {erro && (
        <div className="p-3 rounded bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-100 text-sm whitespace-pre-wrap">
          {erro}
        </div>
      )}

      {!loading && (roteadores?.length ?? 0) === 0 && !erro && (
        <p className="text-gray-600 dark:text-slate-300">Nenhum roteador cadastrado.</p>
      )}
        ...
        {roteadores.map((r) => {
          ...
          return (
            ...
            >
              ...
              {s && (
                <pre className="mt-2 text-[11px] bg-gray-100 text-slate-900 dark:bg-slate-800 dark:text-slate-100 rounded p-2 overflow-x-auto">
                  {jsonPretty(s)}
                </pre>
              )}
            </div>
