...
export async function GET(req: Request) {
  ...
  try {
    ...
    const relayErr = err as { status?: number; data?: unknown; message?: string } | null;
    if (relayErr?.status) {
      return NextResponse.json(relayErr?.data || { ok: false, error: 'relay_error' }, { status: relayErr.status });
    }
    logger.error({ error: relayErr?.message || String(err) }, '[hotspot/active] relay unreachable');
    recordApiMetric('hotspot_active', { durationMs: Date.now() - started, ok: false });
