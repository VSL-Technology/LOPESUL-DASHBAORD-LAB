...
export async function findDeviceRecord({ deviceId, mikId, ip }: DeviceLookup = {}) {
  ...
  if (id) {
    ...
    if (isUUID(id)) {
      ...
      const byId = await prisma.dispositivo.findUnique({ where: { id } }).catch((err: unknown) => {
      ...
      const byMikId = await prisma.dispositivo.findUnique({ where: { mikId: id } }).catch((err: unknown) => {
  ...
  if (mik) {
    ...
    const byMik = await prisma.dispositivo.findUnique({ where: { mikId: mik } }).catch((err: unknown) => {
  ...
  if (deviceIp) {
    ...
    const byIp = await prisma.dispositivo.findFirst({ where: { ip: deviceIp } }).catch((err: unknown) => {
