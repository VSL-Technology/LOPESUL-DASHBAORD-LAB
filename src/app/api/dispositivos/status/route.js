...
export async function GET() {
  ...
  try {
    ...
    if (!auth.session) {
      recordApiMetric('dispositivos_status', {
    ...
    if (auth.maintenance && !auth.isMaster) {
      logger.warn({ role: auth.role }, '[dispositivos/status] bloqueado por manutenção');
      recordApiMetric('dispositivos_status', {
        durationMs: Date.now() - started,
        ok: false,
      });
      return NextResponse.json(
        { error: 'Modo manutenção ativo. Aguarde liberação do operador Master.' },
        { status: 423 }
      );
    }
    ...
    logger.debug(resp.meta, '[dispositivos/status] status aggregated');
    recordApiMetric('dispositivos_status', {
      durationMs: Date.now() - started,
      ok: true,
    });
    ...
    logger.error({ error: e?.message || e }, 'GET /api/dispositivos/status');
    recordApiMetric('dispositivos_status', {
      durationMs: Date.now() - started,
      ok: false,
    });
