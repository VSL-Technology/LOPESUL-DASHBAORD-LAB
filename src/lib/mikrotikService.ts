...
// @deprecated Lógica de Mikrotik deve residir no Relay; manter apenas para compatibilidade enquanto migramos.
...
function requireIdentity(roteador: Roteador) {
  const identity = (roteador as any)?.identity as string | null | undefined;
  if (!identity || typeof identity !== 'string' || !identity.trim()) {
    const err = new Error('Missing router identity (required for Relay calls)');
    (err as any).code = 'MISSING_ROUTER_IDENTITY';
    throw err;
  }
