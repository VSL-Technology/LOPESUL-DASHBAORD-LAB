...
echo ""
echo "📡 Verificando no Mikrotik (${MIKROTIK_IP})..."
echo ""

# Verificar se IP está na lista paid_clients
echo "3️⃣ Lista 'paid_clients':"
PAID_CLIENTS=$(sshpass -p "$MIKROTIK_PASS_VAR" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER_VAR"@"$MIKROTIK_IP" \
...
BINDING=$(sshpass -p "$MIKROTIK_PASS_VAR" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER_VAR"@"$MIKROTIK_IP" \
...
ATIVAS=$(sshpass -p "$MIKROTIK_PASS_VAR" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER_VAR"@"$MIKROTIK_IP" \
...
if [ -n "$ATIVAS" ] && [ "$ATIVAS" != "" ]; then
  ...
  sshpass -p "$MIKROTIK_PASS_VAR" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER_VAR"@"$MIKROTIK_IP" \
