...
import { liberarAcessoPorPedido, revogarAcessoPorSessao } from '@/lib/mikrotik';
import { processPendingReleaseRequests } from '@/lib/auditEventProcessor';
...
async function tick() {
  ...
  // 1) process pending audit event release requests (from webhooks)
  await processPendingReleaseRequests(10).catch((e) =>
    console.error('[scheduler] audit event processor error', e)
  );

  // 2) expirar pendentes
  await expiraPendentes(now);

  // 3) criar sessões para pagos sem sessão + liberar no Mikrotik
