...
MIKROTIK_USER="${MIKROTIK_USER:?Defina MIKROTIK_USER no ambiente/.env}"
MIKROTIK_PASS="${MIKROTIK_PASS:?Defina MIKROTIK_PASS no ambiente/.env}"
...
for IP in "${!MIKROTIKS[@]}"; do
  ...
  # Tentar via SSH
  echo "$REDIRECT_CONTENT" | sshpass -p "$MIKROTIK_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER"@$IP \
  ...
  sshpass -p "$MIKROTIK_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$MIKROTIK_USER"@$IP \
