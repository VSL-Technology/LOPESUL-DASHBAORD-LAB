...
async function main() {
  ...
  const admin = await prisma.operador.upsert({
    where: { nome: 'admin' },
    update: { senha: senhaCriptografada, role: 'MASTER', ativo: true },
    create: { nome: 'admin', senha: senhaCriptografada, role: 'MASTER', ativo: true },
  });
  console.log(`✅ Operador '${admin.nome}' pronto. Senha definida com sucesso.`);
  if (!process.env.SEED_ADMIN_PASSWORD) {
    console.log('⚠️  Senha gerada aleatoriamente; altere após login.');
  }
