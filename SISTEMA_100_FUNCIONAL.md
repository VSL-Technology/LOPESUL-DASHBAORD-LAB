...
> Todas definidas apenas no `.env`/`.env.local` (sem duplicar em docs/scripts).
```bash
MIKROTIK_HOST=<definir-no-env>
MIKROTIK_PORT=<definir-no-env>
MIKROTIK_USER=<definir-no-env>
MIKROTIK_PASS=<definir-no-env>
MIKROTIK_SSL=<definir-no-env>
MIKROTIK_TIMEOUT_MS=<definir-no-env>
RELAY_URL=<definir-no-env>
RELAY_TOKEN=<definir-no-env>
PAGARME_SECRET_KEY=<definir-no-env>
DATABASE_URL=<definir-no-env>
```
